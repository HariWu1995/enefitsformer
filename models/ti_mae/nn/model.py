...
class InferenceEncoder(nn.Module):
    ...
    def __init__(self, in_chans=50,
        ...
        self.blocks = nn.ModuleList([
            nn.TransformerEncoderLayer(embed_dim, num_heads, d_hid, dropout, norm_first=norm_first, batch_first=True)
...
class MaskedAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        if n_clusters:
            self.cluster_layer = SoftKMeans(n_clusters, seq_len=int(seq_len*(1-mask_ratio)),
                                            n_features=embed_dim, dist_metric=dist_metric)
        else:
        ...
        self.decoder_blocks = nn.ModuleList([
            nn.TransformerEncoderLayer(decoder_embed_dim, decoder_num_heads, d_hid, dropout, norm_first=norm_first, batch_first=True)
    ...
    def random_masking(self, x: torch.Tensor, mask_ratio, forecasting_ratio=0.25, forecast_steps=10):
        """
    ...
    def forward_encoder(self, x: torch.Tensor, mask_ratio):
        # embed patches
        x = self.embedder(x)
        # N, L, C = x.shape
    ...
    def forward_loss(self, x: torch.Tensor, 
                        pred: torch.Tensor, 
                        mask: torch.Tensor, latent, forecasting_ratio=0.25, forecast_steps=10):
        """
        x: [N, W, L]
        pred: [N, L, W]
        ...
        if forecasting_ratio:
            ...
            backcast_loss = (mask_forecast * loss_forecast)[:,:-forecast_steps].sum() / mask_forecast[:,:-forecast_steps].sum()      
        else:
            forecast_loss = 0.
            backcast_loss = 0.
    ...
    def forward(self, x: torch.Tensor, y=torch.Tensor([1]).long().cuda()):
    ...
    def predict(self, x: torch.Tensor, pred_samples=5):
