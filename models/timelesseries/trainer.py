...
class TimelessTrainer(Dataset):
    ...
    def __init__(self, model: nn.Module, train_dataset: Dataset, valid_dataset: Dataset, 
                    ...
                    lr: float=0.1, num_epochs: int=1, reproducibity_seed: int=0, 
    ...
    def _print_epoch_stats(self, pbar, curr_step, curr_lr,
                                train_len, train_losses     , train_stats, 
                                valid_len, valid_losses=None, valid_stats=None):

        out_dict = dict()

        if valid_losses:
            out_dict.update({f'val_{k}': v.item() for k, v in valid_losses.items()})

        if valid_stats:
            out_dict.update({f'val_{k}': v.item() for k, v in valid_stats.items()})

        out_dict.update({k: v.item() for k, v in train_losses.items()})
        out_dict.update({k: v.item() for k, v in train_stats.items()})
    ...
    def _train_loop(self, gpu, world_size):
        ...
        for epoch in range(epoch_start, epoch_start+self.num_epochs): 
            ...
            for i in loader:
                ...
                if self.print_stats and master:
                    self._print_epoch_stats(loader, i, model.get_latest_lr(), train_len, losses, model.train_stats(), valid_len)
            ...
            with torch.no_grad():
                for i in range(valid_len):
                    ...
                    # valid step
                    val_losses = model.valid_step(*data)
                    val_loss = val_losses.pop('total')
                    valid_losses.append(val_loss.item())

                    if self.print_stats and master:
                        self._print_epoch_stats(loader, i, model.get_latest_lr(), train_len,     losses, model.train_stats(), 
                                                                                  valid_len, val_losses, model.valid_stats(),)
